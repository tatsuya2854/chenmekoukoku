...
| 項目 | 指定 |
|---|---|
| 見た目の年齢 | **21〜24 歳**（v2：初稿が 25〜27 に見えたため若返り）。10 代に見せない |
...
| ネイル | ミルキーピンク、短め |
| 服（A・B） | **ChenMe ブラッシュピンクの半袖半ズボンのパジャマセット**（コットン）。玄関カットだけヒールが床に置いてある。仕事着で寝室に座らせない（v2） |
| 服（KV 再現時のみ） | サテンのキャミワンピ、KV と同じピンク系 |
| 服（C） | 手だけ実写のため不要 |
...
## 0e. ポンプからクリームが出る瞬間（2026-09-09）
FB「変なところから出ている」。原因：開始フレームでノズルが手のひらの真上にないと、Veo が"出る場所"を勝手に決める。
| 案 | 内容 | 状態 |
|---|---|---|
| A 本命 | 開始フレームを作り直す：ノズルを手のひらの真上 3cm、親指がポンプ頭に乗った構図（`c_v03_stills.py P2b_pump`）。Veo は「cream emerges from the nozzle tip」を明示して **3 テイク**、最良を採用 | プロンプト準備済み。トークン待ち |
| B 保険（現行 v04） | 指が押す 0.8 秒で切り、手にクリームが乗っている静止画へカット。出る瞬間を映さない | 反映済み |
| C 音 | 本番は「しゅっ」の SE が瞬間を補う | 本番で |
